...
// Define interface for Summary based on your model/API response
interface Summary {
  _id: string;
  title: string;
  createdAt: string | Date;
  summary_text: string;
  // Add other fields if accessed elsewhere, e.g., file_name: string;
}

export function SummaryTable() {
  const [summaries, setSummaries] = useState<Summary[]>([]);  // Typed array
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const router = useRouter();
  ...
  const fetchSummaries = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/summaries-list");
      // Type assertion: Assume res.data.data is Summary[]
      setSummaries(res.data.data as Summary[] || []);
    } catch (error) {
      console.error("Error fetching summaries:", error);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {  // Explicit param type
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/summaries-list/${id}`);
      if (res.data) {
        fetchSummaries();  // Refetch to update list
      }
    } catch (error: unknown) {
      console.error("Delete error:", error);
    } finally {
          ...
          <table className="w-full border-collapse table-fixed">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                summaries.map((summary) => (  // Now summary is typed as Summary
                  <tr
                    key={summary._id}
                    className="hover:bg-gray-50 transition-colors"
                  >
                    ...
                    <td className="py-3 px-4 space-x-2">
                      ...
                      <Button 
                        ...
                        disabled={deletingId === summary._id}  // Optional: Disable during delete
                      >
                        {deletingId === summary._id ? "Deleting..." : "Delete"}
                      </Button>
                    </td>
